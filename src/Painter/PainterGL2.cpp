...
void PainterGL2::Square(PositionF position, SizeUI size, Layer layer) {
  glTexCoord2f(0, 1);
...
void PainterGL2::Sprite(PositionF position,
                        SizeUI size,
                        Layer layer,
                        Sprite::Name sprite,
                        int offset) {
  auto coord = Sprite::Coordinates(sprite, offset);
  assert(coord.size() == 4);
  ...
  glTexCoord2f(coord[2]);
  glVertex3f(position.x + size.x, position.y + size.y,
             static_cast<float>(layer));

  glTexCoord2f(coord[3]);
  glVertex3f(position.x, position.y + size.y, static_cast<float>(layer));
}
...
void PainterGL2::Resize(int width, int height) {
  ...
  glError();
  glm::mat4 mat = glm::ortho<float>(-width, 0, -height / 2.0f, height / 2.0f,
                                    -11.0f, 11.0f);
